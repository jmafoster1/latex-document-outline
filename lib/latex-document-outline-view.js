...
export default class LatexDocumentOutlineView {
  ...
  updateStructure() {
    ...
    this.getStructure(text, this.homeDir, this.mainFileName, doc.content);
  }
  ...
  constructor(structure) {
    ...
    this.homeDir = path.dirname(this.mainFile);
    this.mainFileName = path.basename(this.mainFile);
          ...
          if (part) {
            if (part.title) {
              htmlString += `<li>
                              <div>
                                ...
                                <a class="part open" data-line="${part.line}" data-path="${part.file}" href="atom://core/open/file?filename=${part.file}&line=${part.line}">
                                  ${part.title}
                                </a>
                              </div>`;
            }
            if (part.figures) {
              ...
              part.figures.forEach(function(figure){
                htmlString += `<li class="${figure.type}"><a class="open" data-line="${figure.line}" data-path=${figure.file} href="atom://core/open/file?filename=${part.figure}&line=${part.figure}">${figure.title}</a></li>`
              });
              htmlString += `</ul>`
            }
                ...
                if (chapter) {
                  if (chapter.title) {
                    htmlString += `<li>
                                    <div>
                                      ...
                                      <a class="chapter open" data-line="${chapter.line}" data-path="${chapter.file}"  href="atom://core/open/file?filename=${chapter.file}&line=${chapter.line}">
                                        ${chapter.title}
                                      </a>
                                    </div>`;
                  }
                  if (chapter.figures) {
                    ...
                    chapter.figures.forEach(function(figure){
                      htmlString += `<li class="${figure.type}"><a class="open" data-line="${figure.line}" data-path=${figure.file} href="atom://core/open/file?filename=${figure.file}&line=${figure.line}">${figure.title}</a></li>`
                    });
                    htmlString += `</ul>`
                  }
                        ...
                        if (section.title) {
                          htmlString += `<li>
                                          <div>
                                            ...
                                            <a class="section open" data-line="${section.line}" data-path="${section.file}"  href="atom://core/open/file?filename=${section.file}&line=${section.line}">
                                              ${section.title}
                                            </a>
                                          </div>`;
                        }
                            ...
                            if (subsection) {
                              if (subsection.title) {
                                htmlString += `<li>
                                                <div>
                                                  ...
                                                  <a class="subsection open" data-line="${subsection.line}" data-path="${subsection.file}"  href="atom://core/open/file?filename=${subsection.file}&line=${subsection.line}">
                                                    ${subsection.title}
                                                  </a>
                                                </div>`;
                              }
                              if (subsection.figures) {
                                ...
                                subsection.figures.forEach(function(figure){
                                  htmlString += `<li class="${figure.type}"><a class="open" data-line="${figure.line}" data-path=${figure.file}  href="atom://core/open/file?filename=${subsection.file}&line=${subsection.line}">${figure.title}</a></li>`
                                });
                                htmlString += `</ul>`
                              }
                                  ...
                                  if (subsubsection) {
                                    if (subsubsection.title) {
                                      htmlString += `<li>
                                                      <div>
                                                        ...
                                                        <a class="subsubsection open" data-line="${subsubsection.line}" data-path="${subsubsection.file}"  href="atom://core/open/file?filename=${subsubsection.file}&line=${subsubsection.line}">
                                                          ${subsubsection.title}
                                                        </a>
                                                      </div>
                                                    </li>`;
                                    ...
                                    if (subsubsection.figures) {
                                      ...
                                      subsubsection.figures.forEach(function(figure){
                                        htmlString += `<li class="${figure.type}"><a class="open" data-line="${figure.line}" data-path=${subsubsection.file}  href="atom://core/open/file?filename=${figure.file}&line=${figure.line}">${figure.title}</a></li>`
                                      });
                                      htmlString += `</ul>`
                                    }
                                  }
